...
声明中建议推迟的特性包括：数值转换、int 和 uint、十进制、运算符重载、泛型函数（generic function）、`wrap`、堆栈标记、生成器、尾调用、nullability、程序单位、重构的 `with`、修订后的 `eval` 和命名空间过滤器。在说明了这些特性被推迟的原因之后，声明提出了 Adobe 对 ECMAScript 未来发展的修订后意见：
