...
*模块*。ES4 设计的复杂部分之一，就是用于构建大型程序和库的「包和命名空间」结构。当新版 ES4 被放弃时，人们已经发现这些机制存在重大问题。它们显然不适合进入 Harmony。当时有影响力的 JavaScript 开发者们所使用的，还是基于模块模式的特有模块化解决方案。2009 年 1 月，Kris Kowal 和 Ihab Awad 向 TC39 提交了一份受模块模式启发的设计。他们的设计最终演变成了 Node.js 中使用的 CommonJS 模块系统。
