...
对此在术语上的基本需求，是区分具有正常语义的对象和具有反常（即不寻常）语义的对象。Douglas Crockford [[TC39 2012b](./references.md#TC39:2012:034)] 根据 Ecma 最高会员等级的名称，建议用「标准对象」（ordinary object）来表示那些语义上使用 JavaScript 对象字面量或 `new Object()` 来创建的对象。凡是在语义上与普通对象语义有任何偏离的对象，都被称为「外来对象」（exotic object）。标准对象和外来对象都可能由宿主、引擎或应用程序员提供，也可能用 JavaScript 或其他语言来实现。
