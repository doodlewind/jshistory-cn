...
对此的修复方法，是定义一些额外的非严格模式代码规则。这些规则可以静态地检测那些特定的可互操作用例，并使其合法地与遗留网页相兼容。对于上面的例子，规则会把上面的代码当作这样：
...
会后，Mark Miller 创造了一份新的稻草人提案。尽管该提案仍有许多细节缺乏共识，它在冻结特性会议上仍然获得了接受。图 46 中作为示例的类定义，是基于 Miller 的特性冻结类提案而给出的：

``` js
class Monster extends Character {
  constructor(name, health) { // 构造器韩束
    super();             // 调用父类构造器
